...
export class IncomeSourceService {
  ...
  async create(req: RequestContext, createIncomeSourceDto: CreateIncomeSourceDto) {
  ...
  async findAll(req: RequestContext) {
    const { id: userId } = req.user;
  ...
  async findOne(req: RequestContext, id: number) {
    const { id: userId } = req.user;
  ...
  async update(req: RequestContext, id: number, updateIncomeSourceDto: UpdateIncomeSourceDto) {
    const { id: userId } = req.user;
  ...
  async remove(req: RequestContext, id: number) {
    const { id: userId } = req.user;
    const incomeSource = await this.findOne(req, id);
